...
userController.createUser = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      "Error in userController.createUser: verifying if user already exists."
    );
  }
  ...
  try {
    ...
    return next( new Error('Error in userController.createUser'))
  }
};
...
userController.verifyUser = async (req, res, next) => {
  ...
  if (!email || !password || (!email && !password))
    return next(
      "Error in userController.verifyUser: not given all necessary inputs."
    );
  try {
    const sqlCommand = `
      SELECT * FROM users WHERE email = $1
    ...
    return next(new Error('Error in userController.verifyUser'))
  }
};
...
if (req.user && req.user.id !== parseInt(id)) {
  ...
  try {
    ...
    return next(new Error('Error in userController.viewAllDogs'))
  }
};
